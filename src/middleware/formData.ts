...
const PRESETS = {
  profilePhoto: 'carmu_profile_photo_preset',
  category: 'carmu_category_preset',
  product: 'carmu_product_preset',
  tag: 'carmu_tag_preset',
};
...
const formData = (req: Request, _res: Response, next: NextFunction) => {
  ...
  bb.on('file', (key, file, info) => {
    ...
    else if (originalUrl.includes('customers')) preset = PRESETS.profilePhoto;
    else if (originalUrl.includes('employees')) preset = PRESETS.profilePhoto;
